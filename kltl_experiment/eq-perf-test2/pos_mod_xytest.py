...
URI='radio://0/80/2M/E7E7E7E705' # We are currently working with drone 5
dt = 5.0
alpha = 0 # alpha = {0, pi/4}
W_coef = 1 # W_coef = {1, 0.5}
...
mat_file_name = "data/controllers/scalar_controller2_data_13Mar2023-1700.mat"
...
if __name__ =='__main__':
    ...
    hidden_mode = np.random.randint(0,2) 
    ...
    with SyncCrazyflie(URI, cf=Crazyflie(rw_cache='./cache')) as scf:
        ...
        with SyncLogger(scf, log_est) as logger:
            ...
            x_next = hm_dynamics.f(x0,np.reshape(u,(2,1)),w*W_coef)
            print("x_next = ", x_next)
            ...
            while i < TimeHorizon:
                ...
                try:
                    ...
                    w = np.reshape(sample_from_polytope(hm_dynamics.W),newshape=(hm_dynamics.dim_w(),1))
                    x_next = hm_dynamics.f(x_t,np.reshape(u,(2,1)),w*W_coef)
                    print("x_next = ",x_next)
                    i += 1
                    print(f'i is {i}')
                    t = time.time()
