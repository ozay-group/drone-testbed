...
def faultDetect(arr):
    ...
    n0 = LpVariable.matrix("n0", list(range(3)), -0.05, 0.05)
    n1 = LpVariable.matrix("n1", list(range(3)), -0.05, 0.05)
    n2 = LpVariable.matrix("n2", list(range(3)), -0.05, 0.05)

    # Use None for +/- Infinity, i.e. z <= 0 -> LpVariable("z", None, 0)

    A1 = np.matrix('0.5 0.5 0; 0.5 0.5 0; 0 0 1')
    A2 = np.matrix('1 0 0; 0 0.5 0.5; 0 0.5 0.5')
